...
int main(void)
{
                                    ...
                                    // 'n' (pyramid height)
    {
        ...
        printf("  ");                   // Prints a double space after the first half of the pyramid
        for (int k = 0; k < brick; k++) // Second brick loop to print other half of pyramid
        {
            printf("#");    // Prints a single # per loop
        }
        printf("\n");       // Prints a new line
        brick += 1;         // Increments 'brick' to add another '#' to the following layer
        space -= 1;         // Decrements 'space' to remove one ' ' from the following layer to make room
                            // for new '#'
    }
}
